...
def download_compressed_data(folder_to_download, path_to_save, name_joblib_downloaded):
    print(f"Started downloading from {folder_to_download}")
    start_time = time.time()
    paths = file_system_client.get_paths(folder_to_download)
    ...
    for path in paths:
        ...
        if path_name.endswith('.json.gz')  and path_name not in downloaded_data:
            file_client = file_system_client.get_file_client(path_name)
